...
# -*- coding: utf-8 -*-
def song_word(key):
    url = f"http://api.tianapi.com/zmsc/index?key=2ac09d3b2c7b17ec43198b1840ea14de"
...
def story(key):
    url = f"http://api.tianapi.com/story/index?key=2ac09d3b2c7b17ec43198b1840ea14de"
    ret = requests.get(url)
    ret = ret.content.decode('utf8').replace("'", '"')
    data_json = json.loads(ret)
...
def star(key, star_name):
    url = f"http://api.tianapi.com/star/index?key=2ac09d3b2c7b17ec43198b1840ea14de&astro=狮子座"
    ret = requests.get(url)
    ret = ret.content.decode('utf8').replace("'", '"')
    data_json = json.loads(ret)
...
def health(key):
    url = f"http://api.tianapi.com/healthtip/index?key=2ac09d3b2c7b17ec43198b1840ea14de"
    ret = requests.get(url)
    ret = ret.content.decode('utf8').replace("'", '"')
    data_json = json.loads(ret)
...
def night_msg(key):
    url = f"http://api.tianapi.com/wanan/index?key=2ac09d3b2c7b17ec43198b1840ea14de"
    ret = requests.get(url)
    ret = ret.content.decode('utf8').replace("'", '"')
    data_json = json.loads(ret)
    msg = data_json['newslist'][-1]['content']
...
def morning_msg(key):
    url = f"http://api.tianapi.com/zaoan/index?key=2ac09d3b2c7b17ec43198b1840ea14de"
    ret = requests.get(url)
    ret = ret.content.decode('utf8').replace("'", '"')
    data_json = json.loads(ret)
    msg = data_json['newslist'][-1]['content']
...
if __name__ == "__main__":
    ...
    tianapi_api = '2ac09d3b2c7b17ec43198b1840ea14de'  # 第三方接口的key
    """从微信公众平台获取的相关密钥"""
    client_info = {
        "APP_ID": 'wxc5d81149fa3d9483',
        "APP_SECRET": '9339914193db91533d4a21a9e7ad2d92',
        "USER_ID": ['o97SB6czLV_jZrh95G0ao5nrfIck', 'o97SB6SvjiP_wnOpBAuf3VXYcQ0k'],
        "TEMPLATE_ID": {
            'daily_id1': 'VjoYdC-GfuSfALRbEd_3vh2YDWoSC12JpyCtsoxqu5c',
            'daily_id2': 'VZBCUugN7wlHBnO6C0BVwTK-ubUkXkWyb1y9kpkD4kw',
            'star_id':   'G9TMBSyH5bf177eUyHjmLHLShJqDcwRVV4t7PUBywbE',
            'menses_id': '6GIo4YasJ1MXADbic4djAFCpvNBd6Yg_gwW9uX2EO2E',
            'night_id':  '7XKX4oi2Xdj4IxQs92MaZ5-OsPg25Fz5ze8EA991oR4',
            'story_id':  'c8gco4BiN7kQ1HMT7RCIsyuVqJ7klBAJhPqPPXx7T2U',
        }
    }
